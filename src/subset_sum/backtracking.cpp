...
namespace assignment {
  ...
  void SubsetSumBacktracking::search(const std::vector<int>& set, int index, int mask, int sum, int residual,
    ...
    search(set, index, mask,std::accumulate(subset.begin(), subset.end(), 0),residual, target_sum, indices);
    search(set, index, set_bit(mask, index),std::accumulate(subset.begin(),subset.end(), 0) + set[index],residual, target_sum, indices);
  }
